...
<html lang="en">
    <head>
        <title>Others Policy List</title>
        <meta charset="utf-8">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    <c:if test="${User.role.equals('2')}">
                                        <c:if test="${AllPendingEducationalPlanClaims.size()!=0}">
                                            <c:forEach items="${AllPendingEducationalPlanClaims}" var="claim">
                                                <tr data-index="0">
                                ...
                                <c:if test="${User.role.equals('3')}">
                                    <c:if test="${AllApprovedEducationalPlanClaims.size()!=0}">
                                        <c:forEach items="${AllApprovedEducationalPlanClaims}" var="claim">
                                            <tr data-index="0">
                                                <td><c:out value="${claim.medicalHistory}"> </c:out></td>
                                                <td><c:out value="${claim.relation}"> </c:out>
                                                <td><c:out value="${claim.dob}"> </c:out></td>
